...
# ================= CONFIG =================
VIDEO_PATH = "Full/test.mp4"   # <-- Your video file
...
frame_count = 0
last_boxes = None
previous_state = 0  # To avoid continuous Firebase writes

print("[SYSTEM] Processing video... Press Q to exit")
...
while True:
    ...
    # -------- Firebase Update (Only on State Change) --------
    ...
    # -------- Alert Text --------
    if knife_detected == 1:
        cv2.putText(detected_frame, "ALERT: ANOMALY DETECTED",
                    (20, 40),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1,
                    (0, 0, 255),
    ...
    # -------- Show Final Window Only --------
    cv2.imshow("DarkSightAI - Video Monitoring", detected_frame)
